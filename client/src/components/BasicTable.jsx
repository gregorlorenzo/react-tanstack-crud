              ...
              <tr key={headerGroup._id}>
                {headerGroup.headers.map((header) => (
                  <th key={header._id} scope='col' className='px-6 py-3' onClick={header.column.getToggleSortingHandler()}>
                    ...
                    {
                      { asc: '⬆️', desc: '⬇️' }[header.isSorted ?? null]
                    }
                  </th>
                ))}
              </tr>
      ...
      <nav className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4">
        <div className="flex justify-start gap-2 mt-4">
          <button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-2.5 me-2"
            onClick={() => table.gotoPage(0)}>First Page</button>
          <button className={`text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-2.5 me-2 ${!table.canPreviousPage ? 'opacity-50' : ''}`}
            onClick={() => table.previousPage()} disabled={!table.canPreviousPage}>Previous Page</button>
          <button className={`text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-2.5 me-2 ${!table.canNextPage ? 'opacity-50' : ''}`}
            onClick={() => table.nextPage()} disabled={!table.canNextPage}>Next Page</button>
          <button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-2.5 me-2"
            onClick={() => table.gotoPage(table.pageCount - 1)}>Last Page</button>
        </div>
      </nav>
